...
class Agent():
    ...
    def __init__(self, state_size, action_size, seed, double_dqn=False,dueling_dqn=False):
        ...
        ======
            ...
            double_dqn: implement double deep q-learning
            dueling: implement dueling deep q-learning
        """
    ...
    def compute_loss(self,Q_expected, Q_targets,deltas):
    ...
    def get_qnetwork_target(self,next_states,rewards,gamma,dones):
        """ Returns the target expected Q value  
  
        Params
        ======
            next_states : list of states 
            rewards : rewards obtained
            gamma : discounting factor
            dones : list of bools indicating if it is finished
        """
    ...
    def learn(self, experiences, gamma):
        ...
        Params
        ======
            experiences (Tuple[torch.Variable]): tuple of (s, a, r, s', done, deltas) tuples 
            gamma (float): discount factor
        """
        states, actions, rewards, next_states, dones,deltas = experiences

        # Get the q-network target expected 
        Q_targets = self.get_qnetwork_target(next_states,rewards,gamma,dones)

        # Get expected Q values from local model
        Q_expected = self.qnetwork_local(states).gather(1, actions)
        # Compute loss
        loss = self.compute_loss(Q_expected, Q_targets,deltas)
        # Minimize the loss
